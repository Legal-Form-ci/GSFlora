...
import {
  ...
  Search, MoreHorizontal, Edit, Trash2, Shield, Loader2, UserPlus,
} from 'lucide-react';
...
const roleLabels: Record<AppRole, string> = {
  super_admin: 'Super Admin', admin: 'Administrateur', founder: 'Fondateur',
  director: 'Directeur', censor: 'Censeur', educator: 'Éducateur',
  principal_teacher: 'Prof. Principal', teacher: 'Enseignant',
  student: 'Élève', parent: 'Parent',
};

const roleColors: Record<AppRole, string> = {
  super_admin: 'bg-red-100 text-red-800', admin: 'bg-purple-100 text-purple-800',
  founder: 'bg-amber-100 text-amber-800', director: 'bg-indigo-100 text-indigo-800',
  censor: 'bg-cyan-100 text-cyan-800', educator: 'bg-teal-100 text-teal-800',
  principal_teacher: 'bg-violet-100 text-violet-800', teacher: 'bg-blue-100 text-blue-800',
  student: 'bg-green-100 text-green-800', parent: 'bg-yellow-100 text-yellow-800',
};
...
const UsersManagement = () => {
  ...
  const DEFAULT_PASSWORD = 'SchoolHub2026';
  const [formData, setFormData] = useState({
    email: '', password: DEFAULT_PASSWORD, first_name: '', last_name: '',
    phone: '', role: 'student' as AppRole,
  });
    ...
    try {
      ...
      if (authData.user) {
        ...
        await supabase.from('user_roles').insert({
          user_id: authData.user.id, role: formData.role as any,
        });
        ...
        // Create user settings with must_change_password
        await supabase.from('user_settings').insert({
          user_id: authData.user.id, must_change_password: true,
        });
  ...
  const handleEditUser = async () => {
    ...
    try {
      ...
      await supabase
        .from('user_roles')
        .upsert({ user_id: selectedUser.id, role: formData.role as any }, { onConflict: 'user_id,role' });
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      // Remove from school_members (soft delete)
      if (currentSchool) {
        await supabase
          .from('school_members')
      ...
      toast.success('Utilisateur retiré de l\'établissement');
      fetchUsers();
    } catch (error) {
  ...
  const openEditModal = (user: User) => {
    ...
    setFormData({
      ...
      last_name: user.last_name, phone: user.phone || '', role: user.role || 'student',
    });
    setShowEditModal(true);
  };

  const resetForm = () => {
    setFormData({ email: '', password: DEFAULT_PASSWORD, first_name: '', last_name: '', phone: '', role: 'student' });
    setSelectedSubjects([]);
    setPrimarySubject('');
    ...
    <DashboardLayout navItems={navItems} title="Gestion des utilisateurs">
        ...
        <div className="flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
          <div className="flex flex-1 gap-4">
            ...
            <Select value={roleFilter} onValueChange={setRoleFilter}>
              <SelectTrigger className="w-48"><SelectValue placeholder="Filtrer par rôle" /></SelectTrigger>
              <SelectContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Users className="w-5 h-5" /> Utilisateurs ({filteredUsers.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
        ...
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input type="text" value={formData.password} disabled className="bg-muted text-muted-foreground" />
              <p className="text-xs text-muted-foreground">⚠️ L'utilisateur devra changer son mot de passe à la première connexion.</p>
            </div>
            <div className="space-y-2">
      ...
      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setShowEditModal(false); resetForm(); }}>Annuler</Button>

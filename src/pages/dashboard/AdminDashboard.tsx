...
import {
  Home, Users, GraduationCap, BookOpen, Settings, BarChart3, Calendar, Bell,
  Shield, UserPlus, ChevronRight, TrendingUp, DollarSign,
} from 'lucide-react';
...
interface RecentUser {
  id: string; email: string; first_name: string; last_name: string; created_at: string;
}
...
const AdminDashboard = () => {
  ...
  const [stats, setStats] = useState({
    ...
    totalClasses: 0, totalCourses: 0, totalSubjects: 0,
  });
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const { data: members } = await supabase
        .from('school_members').select('user_id, role')
        .eq('school_id', currentSchool.id).eq('is_active', true);
      ...
      const [classesRes, subjectsRes, coursesRes] = await Promise.all([
        supabase.from('classes').select('*', { count: 'exact', head: true }).eq('school_id', currentSchool.id),
        supabase.from('subjects').select('*', { count: 'exact', head: true }).eq('school_id', currentSchool.id),
        supabase.from('courses').select('*', { count: 'exact', head: true }).eq('school_id', currentSchool.id),
      ]);

      // Recent users
      let recentUsersData: RecentUser[] = [];
      if (memberIds.length > 0) {
        const { data } = await supabase
          .from('profiles').select('id, email, first_name, last_name, created_at')
      ...
      setStats({
        totalUsers: memberIds.length, totalStudents: studentCount, totalTeachers: teacherCount,
  ...
  return (
    <DashboardLayout navItems={navItems} title={role === 'super_admin' ? 'Super Administration' : 'Administration'}>
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold font-display">
                {currentSchool?.name || 'Administration'}
              </h2>
              <p className="text-primary-foreground/80 mt-1">
                Gérez votre établissement depuis ce panneau
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          <StatCard title="Utilisateurs" value={stats.totalUsers} icon={<Users className="w-6 h-6" />} iconClassName="bg-primary/10 text-primary" />
        ...
        <div className="flex flex-wrap gap-3">
          <Button asChild><Link to="/admin/users/new"><UserPlus className="w-4 h-4 mr-2" /> Nouvel utilisateur</Link></Button>
          <Button variant="outline" asChild><Link to="/admin/classes/new"><GraduationCap className="w-4 h-4 mr-2" /> Nouvelle classe</Link></Button>
          <Button variant="outline" asChild><Link to="/admin/announcements/new"><Bell className="w-4 h-4 mr-2" /> Nouvelle annonce</Link></Button>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <Button variant="ghost" size="sm" asChild><Link to="/admin/users">Voir tout <ChevronRight className="w-4 h-4 ml-1" /></Link></Button>
            </CardHeader>
            <CardContent>
              {loading ? (
                    ...
                    <div key={u.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-muted transition-colors">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                          <span className="text-sm font-medium text-primary">{u.first_name?.[0]}{u.last_name?.[0]}</span>
                        </div>
                        <div>
                ...
                <div className="text-center py-8 text-muted-foreground">
                  <Users className="w-12 h-12 mx-auto mb-3 opacity-50" /><p>Aucun utilisateur</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader><CardTitle className="text-lg font-semibold">Aperçu de l'activité</CardTitle></CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 rounded-lg bg-flora-success/10">
                  <div className="flex items-center gap-3"><TrendingUp className="w-5 h-5 text-flora-success" /><span className="font-medium">Cours publiés</span></div>
                  <Badge variant="secondary" className="bg-flora-success/20 text-flora-success">{stats.totalCourses}</Badge>
                </div>
                <div className="flex items-center justify-between p-4 rounded-lg bg-primary/10">
                  <div className="flex items-center gap-3"><GraduationCap className="w-5 h-5 text-primary" /><span className="font-medium">Classes actives</span></div>
                  <Badge variant="secondary" className="bg-primary/20 text-primary">{stats.totalClasses}</Badge>
                </div>
              </div>
            </CardContent>
          </Card>

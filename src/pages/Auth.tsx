  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        if (error) toast.error(error.message);
        else { toast.success("Compte créé ! Vérifiez votre email pour confirmer."); setMode('login'); }
      } else if (mode === 'forgot') {
    ...
    <div className="min-h-screen bg-gradient-hero flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative z-10">
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="firstName" placeholder="Jean" className={`pl-10 ${errors.firstName ? 'border-destructive' : ''}`}
                ...
                <div className="space-y-2">
                  ...
                  <Input id="lastName" placeholder="Kouamé" className={errors.lastName ? 'border-destructive' : ''}
            ...
            {mode === 'signup' && (
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="phone" type="tel" placeholder="+225 07 00 00 00 00" className="pl-10"
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="email" type="email" placeholder="exemple@email.com" className={`pl-10 ${errors.email ? 'border-destructive' : ''}`}
            ...
            <Button type="submit" className="w-full" size="lg" disabled={loading}>
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                <>{mode === 'login' && "Se connecter"}{mode === 'signup' && "Créer mon compte"}{mode === 'forgot' && "Envoyer le lien"}</>
              )}
            </Button>
            ...
            <div className="text-center pt-4 border-t border-border">
              {mode === 'login' && (
                <>
                  <p className="text-muted-foreground text-sm">Pas encore de compte ?</p>
                  <button type="button" onClick={() => setMode('signup')} className="text-primary font-medium hover:underline mt-1">Créer un compte</button>
                </>
              )}
              {mode === 'signup' && (
                <>
                  <p className="text-muted-foreground text-sm">Déjà un compte ?</p>
                  <button type="button" onClick={() => setMode('login')} className="text-primary font-medium hover:underline mt-1">Se connecter</button>
                </>
              )}
              {mode === 'forgot' && (
                <button type="button" onClick={() => setMode('login')} className="text-primary font-medium hover:underline">Retour à la connexion</button>
              )}
            </div>
          </form>
        </div>
        ...
        <p className="text-center text-white/60 text-sm mt-6">
          © {new Date().getFullYear()} SchoolHub Pro — Plateforme éducative SaaS
        </p>
      </div>
    </div>
  );

...
const DirectorDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const [classesRes, subjectsRes, announcementsRes] = await Promise.all([
      ...
      const gradesRes = await supabase.from('grades').select('score, max_score').eq('school_id', schoolId);
      let averageGrade = 0;
      ...
      setStats({
        totalStudents: members?.filter(m => m.role === 'student').length || 0,
        totalTeachers: members?.filter(m => m.role === 'teacher').length || 0,
    ...
    <DashboardLayout navItems={navItems} title="Tableau de bord - Directeur">
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          <StatCard title="Élèves" value={stats.totalStudents} icon={<GraduationCap className="w-5 h-5" />} />
          <StatCard title="Enseignants" value={stats.totalTeachers} icon={<Users className="w-5 h-5" />} />
          <StatCard title="Classes" value={stats.totalClasses} icon={<School className="w-5 h-5" />} />
          <StatCard title="Matières" value={stats.totalSubjects} icon={<BookOpen className="w-5 h-5" />} />
          <StatCard title="Taux de présence" value={`${stats.attendanceRate}%`} icon={<UserCheck className="w-5 h-5" />} />
          <StatCard title="Moyenne générale" value={`${stats.averageGrade}/20`} icon={<TrendingUp className="w-5 h-5" />} />
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader><CardTitle className="text-lg flex items-center gap-2"><Clock className="w-5 h-5" /> Actions rapides</CardTitle></CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-3">
                <Button asChild variant="outline" className="h-auto py-4 flex-col gap-2">
                  <Link to="/director/schedule-generator"><CalendarDays className="w-6 h-6" /><span className="text-xs">Générer EDT</span></Link>
                </Button>
                <Button asChild variant="outline" className="h-auto py-4 flex-col gap-2">
                  <Link to="/director/users"><Users className="w-6 h-6" /><span className="text-xs">Utilisateurs</span></Link>
                </Button>
                <Button asChild variant="outline" className="h-auto py-4 flex-col gap-2">
                  <Link to="/director/statistics"><BarChart3 className="w-6 h-6" /><span className="text-xs">Statistiques</span></Link>
                </Button>
                <Button asChild variant="outline" className="h-auto py-4 flex-col gap-2">
                  <Link to="/director/announcements"><Megaphone className="w-6 h-6" /><span className="text-xs">Annonces</span></Link>
                </Button>
              </div>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-lg flex items-center gap-2"><Megaphone className="w-5 h-5" /> Annonces récentes</CardTitle>
            <Button asChild variant="ghost" size="sm"><Link to="/director/announcements">Voir tout</Link></Button>
          </CardHeader>
          <CardContent>
            {loading ? (

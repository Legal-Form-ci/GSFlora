...
const StudentSchedule = () => {
  ...
  useEffect(() => {
    if (user && currentSchool) fetchSchedule();
  }, [user, currentSchool]);
            ...
            {Object.entries(groupedByDay).sort(([a], [b]) => Number(a) - Number(b)).map(([day, items]) => (
              <Card key={day}>
                <CardHeader className="pb-3">
                  <CardTitle className="text-lg flex items-center gap-2"><Calendar className="w-5 h-5 text-primary" />{DAYS[Number(day)] || `Jour ${day}`}</CardTitle>
                </CardHeader>
                <CardContent>
                    ...
                    {items.map((item: any) => (
                      <div key={item.id} className="flex items-center gap-4 p-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
                        ...
                        <div className="flex-1 border-l-2 border-primary pl-3">
                          ...
                          <p className="text-sm text-muted-foreground">{item.courses?.profiles?.first_name} {item.courses?.profiles?.last_name}</p>
                        </div>

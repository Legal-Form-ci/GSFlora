...
const StudentGrades = () => {
  ...
  useEffect(() => {
    if (user && currentSchool) fetchGrades();
  }, [user, currentSchool]);
  ...
  const average = filtered.length > 0 ? filtered.reduce((sum, g) => sum + (g.score / g.max_score) * 20, 0) / filtered.length : 0;
  ...
  return (
    <DashboardLayout navItems={studentNavItems} title="Mes Notes">
      <div className="space-y-6">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <Card className="flex-1 min-w-[200px]">
            <CardContent className="py-4 flex items-center gap-4">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center ${average >= 10 ? 'bg-primary/20' : 'bg-destructive/20'}`}>
                {average >= 10 ? <TrendingUp className="w-6 h-6 text-primary" /> : <TrendingDown className="w-6 h-6 text-destructive" />}
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Moyenne générale</p>
                <p className={`text-2xl font-bold ${average >= 10 ? 'text-primary' : 'text-destructive'}`}>{average.toFixed(2)}/20</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><BarChart3 className="w-5 h-5" />Moyennes par matière</CardTitle></CardHeader>
          <CardContent>
                  ...
                  return (
                    <div key={name}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`font-bold ${avg >= 10 ? 'text-primary' : 'text-destructive'}`}>{avg.toFixed(2)}/20</span>
                      </div>
            ...
            {loading ? (
              ...
              <Table>
                <TableHeader><TableRow>
                  <TableHead>Matière</TableHead><TableHead>Type</TableHead><TableHead>Note</TableHead><TableHead>Coef</TableHead><TableHead>Date</TableHead><TableHead>Commentaire</TableHead>
                ...
                <TableBody>
                  {filtered.length === 0 ? (
                    ...
                    <TableRow key={g.id}>
                      ...
                      <TableCell className={`font-bold ${(g.score / g.max_score) >= 0.5 ? 'text-primary' : 'text-destructive'}`}>{g.score}/{g.max_score}</TableCell>
